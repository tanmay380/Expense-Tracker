...
class ICICIParser : BankParser {
    ...
    override fun parse(sender: String, message: String): ParsedTransaction {
        ...
        val isFromCard = message.contains("ICICI Bank Card", ignoreCase = true)
        ...
        return if (isFromCard) {
            ...
            val merchant = if (message.contains("Savings Account", ignoreCase = true)) {
                SmsExtractors.extractMerchantFromSavings(message)
            } else {
                SmsExtractors.extractMerchantFromAccount(message)
            }
            val accountNumber = SmsExtractors.extractAccountNumber(message)
            val bankName = if (message.contains("Savings Account", ignoreCase = true)) {
